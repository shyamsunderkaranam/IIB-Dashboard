...
function MasterCtrl(t,e,r){var o=992;t.getWidth=function(){return window.innerWidth},t.$watch(t.getWidth,function(g){t.toggle=g>=o?angular.isDefined(e.get("toggle"))?e.get("toggle")?!0:!1:!0:!1}),t.toggleSidebar=function(){t.toggle=!t.toggle,e.put("toggle",t.toggle)},window.onresize=function(){t.$apply()}
	t.brokerOptions = false;
	t.valueReveal = false;
	r.selectedBrokerName = "";
	t.brokerlist = [
	  {"id":1,"brokerName":"FIIBINST1"},
	  {"id":2,"brokerName":"FIIBINST2"}
	  ];
	t.openBroker = function(id){
		t.brokerOptions = true;
		r.selectedBrokerName = t.brokerlist[id-1].brokerName;
	 alert("You clicked on Broker "+t.brokerlist[id-1].brokerName);
	};
	t.openQueue = function(){
		t.valueReveal = true;
		alert("valueReveal ::"+t.valueReveal+"r.selectedBrokerName" + r.selectedBrokerName);
	};
	...
	t.propertyValue = [{"InhibitPut":false,"Description":"","Sharable":true,"OpenInputCount":1,"InhibitGet":true,"TriggerControl":false,"MaximumMessageLength":4194304,"OpenOutputCount":0,"CurrentDepth":0,"MaximumDepth":5000}];

}
angular.module("RDash").controller("MasterCtrl",["$scope","$cookieStore","$rootScope",MasterCtrl
  ]);
